...
1. Check the failing test's `test.describe` title (e.g. `"DS-1 Create New Academic Program"`, `"DS-2 Edit Existing Program Details"`)
2. Check the matching `features/DS-N.feature` file name
3. Ask the user if the key is still unclear

## Reproduce before filing

1. Re-run the failing test: `npx playwright test -g "<test title or TC tag>" --workers=1`
...
4. Create the sub-task with `createJiraIssue`:
   - `projectKey`: `DS` (or `JIRA_PROJECT_KEY` from `.env`)
   - `issueTypeName`: `Sub-task`
   - `parent`: parent story key (e.g. `DS-2`)
   - `summary`: `[Composer] <defect title>`
...
- `ATLASSIAN_API_TOKEN` (or `JIRA_API_TOKEN`) — create at [id.atlassian.com](https://id.atlassian.com/manage-profile/security/api-tokens)
...
- DS-1 tests: `tests/ds1-create-program.spec.ts` (project `ds-1`)
- DS-2 tests: `tests/ds2-edit-program.spec.ts` (project `ds-2`)
