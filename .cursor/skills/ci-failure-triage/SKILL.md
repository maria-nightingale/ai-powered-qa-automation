...
description: >-
  Investigates red CI builds and failed GitHub Actions checks. Pulls run logs
  and the playwright-report artifact via gh CLI, reads the Playwright error
  and trace, cross-references the spec, POM, and feature files, classifies
  real app bug vs test issue, and posts a structured diagnosis to the PR.
...
3. Cross-reference: the spec, the POM, and acceptance criteria in this repo.
...
- For a real defect (should be confirmed by human → make request to human user), reuse [jira-bug-reporter](../jira-bug-reporter/SKILL.md) and link the story.
