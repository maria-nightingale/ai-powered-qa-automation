...
1. Read the referenced Jira ticket using the Atlassian MCP. Extract the title, description, and every acceptance criterion.
...
6. End the file with a comment block listing any ambiguities or gaps found in the ticket's acceptance criteria, so the QA can resolve them.

## Output

Save as `features/<ticket-key>.feature`.

Do not write Playwright code in this step unless the user explicitly asks for automation.

## Atlassian MCP

Use the Atlassian MCP server. Read tool schemas before calling (Cursor: `mcps/plugin-atlassian-atlassian/tools/`; Claude Code: available MCP tool descriptors for the configured Atlassian server).

- Resolve `cloudId` via `getAccessibleAtlassianResources`, or pass the site hostname from a Jira URL (e.g. `legionqaschool.atlassian.net`).
- Fetch the ticket with `getJiraIssue`:
  - `issueIdOrKey`: ticket key (e.g. `DS-1`)
  - `responseContentFormat`: `markdown`
- Parse acceptance criteria from description, custom fields, or linked Confluence pages. If ACs are embedded as Gherkin, treat them as minimum coverage — still add negative and edge scenarios.
