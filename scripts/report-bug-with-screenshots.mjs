 ...
 *   node scripts/report-bug-with-screenshots.mjs DS-173 DS-1 "TC-011" tests/ds1-create-program.spec.ts ds-1
 */
...
const project = process.argv[6] || (storyKey === "DS-2" ? "ds-2" : "ds-1");
...
if (!issueKey || !storyKey || !testPattern) {
  fail(
    "Usage: node scripts/report-bug-with-screenshots.mjs <issue-key> <story-key> <test-pattern> [spec-file] [project]",
  );
}

console.log(`Running test matching "${testPattern}" in ${specFile} (project ${project})...`);
spawnSync(
  "npx",
  ["playwright", "test", specFile, "-g", testPattern, "--workers=1", `--project=${project}`],
