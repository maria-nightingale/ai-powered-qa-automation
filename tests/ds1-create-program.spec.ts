...
test.describe('DS-1 Create New Academic Program', () => {
  ...
  test.describe('Positive flows', () => {
    ...
    test('TC-002 — New program is created and appears in the program list', async () => {
    ...
    test('TC-003 — Program can be created with Program Name only', async () => {
  ...
  test.describe('Negative flows', () => {
    ...
    test('TC-009 — Duplicate program name is rejected', async () => {
      const programName = uniqueName('Web Development 2026');

      await programs.createProgram(programName, 'Original program');
  ...
  test.describe('Edge cases', () => {
    test('TC-011 — Program Name at minimum valid length (1 character)', async () => {
      const programName = uniqueName('A');

      await programs.createProgram(programName, 'Single-character name boundary test');

      await programs.expectProgramInList(programName);
    });

    test('TC-012 — Program Name at maximum allowed length', async () => {
      const suffix = Date.now().toString();
      const programName = `${suffix}${repeatChar('A', 255 - suffix.length)}`;

      await programs.createProgram(programName, 'Max length boundary test');
    ...
    test('TC-014 — Special characters in Program Name are handled correctly', async () => {
      const programName = uniqueName('C++ & C#: "Intro" (2026) — 100% Online');

      await programs.createProgram(programName, 'Special characters validation test');
      await programs.expectProgramInList(programName);
    });

    test('TC-015 — Unicode and international characters in Program Name', async () => {
    ...
    test('TC-016 — Leading and trailing spaces are trimmed from Program Name', async () => {
    ...
    test('TC-017 — Description at maximum allowed length', async () => {
    ...
    test('TC-019 — Double-click on Create does not create duplicate programs', async () => {
    ...
    test('TC-020 — Program list updates without manual page refresh', async ({ page }) => {
      const programName = uniqueName('Game Development 2026');
      const urlBefore = page.url();

      await programs.createProgram(programName, 'Unity and Unreal Engine track');
