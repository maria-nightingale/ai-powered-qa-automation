...
test.describe('DS-2 Edit Existing Program Details', () => {
  ...
  test.describe('Positive flows', () => {
    ...
    test('TC-002 — Program name update is saved and reflected in the list', async () => {
      const programName = uniqueName('Web Development 2026');
    ...
    test('TC-004 — Both Name and Description can be updated in a single save', async () => {
      ...
      await programs.createProgram(programName, 'iOS and Android development track');
      await programs.openEditForm(programName);
    ...
    test('TC-005 — Canceling edit discards unsaved changes', async () => {
      const programName = uniqueName('Cybersecurity Essentials');
      const unsavedName = `${programName} — Pro`;

      await programs.createProgram(programName, 'Security fundamentals track');
      await programs.openEditForm(programName);
  ...
  test.describe('Negative flows', () => {
    test('TC-006 — Empty Name prevents save', async () => {
      const programName = uniqueName('UX Design Bootcamp');

      await programs.createProgram(programName, 'Design thinking bootcamp');
      await programs.openEditForm(programName);
    ...
    test('TC-007 — Whitespace-only Name is rejected', async () => {
      const programName = uniqueName('Cloud Computing 2026');

      await programs.createProgram(programName, 'AWS and Azure fundamentals');
      await programs.openEditForm(programName);
    ...
    test.only('TC-008 — Duplicate program name is rejected on edit', async () => {
      const existingName = uniqueName('Web Development 2026');
      const programToRename = uniqueName('Game Development 2026');

      await programs.createProgram(existingName, 'Original program');
      await programs.createProgram(programToRename, 'Game dev track');
    ...
    test('TC-011 — Server error during save does not corrupt program data', async ({ page }) => {
      const programName = uniqueName('DevOps Engineering');
      const updatedName = `${programName} — Updated`;

      await programs.createProgram(programName, 'CI/CD and infrastructure');
      await programs.openEditForm(programName);
      await programs.fillEditForm(updatedName);
    ...
    test('TC-012 — Saving with no changes does not cause errors or duplicate records', async () => {
      const programName = uniqueName('Blockchain Fundamentals');

      await programs.createProgram(programName, 'Distributed ledger technology program');
      await programs.openEditForm(programName);
      await programs.saveEditAndClose();
  ...
  test.describe('Edge cases', () => {
    test('TC-013 — Name at maximum allowed length can be saved', async () => {
      ...
      await programs.createProgram(programName, 'ML certificate track');
      await programs.openEditForm(programName);
    ...
    test('TC-014 — Name exceeding maximum length is rejected', async () => {
      const programName = uniqueName('AI Ethics Program');
      const overLimitName = repeatChar('B', 256);

      await programs.createProgram(programName, 'Ethics in AI systems');
      await programs.openEditForm(programName);
    ...
    test('TC-015 — Special characters in Name are handled correctly on edit', async () => {
      ...
      await programs.createProgram(programName, 'Introductory C programming');
      await programs.openEditForm(programName);
      await programs.fillEditForm(updatedName);
      await programs.saveEditAndClose();
    ...
    test('TC-016 — Unicode characters in Description are preserved on edit', async () => {
      const programName = uniqueName('Global Business Program');
      const updatedDescription =
        'Programme global — グローバルビジネス — Développement international';

      await programs.createProgram(programName, 'International business overview');
      await programs.openEditForm(programName);
      await programs.fillEditForm(undefined, updatedDescription);
      await programs.saveEditAndClose();
    ...
    test('TC-017 — Leading and trailing spaces are trimmed from edited Name', async () => {
      const programName = uniqueName('Network Security Program');
      const trimmedName = `${programName} — Advanced`;

      await programs.createProgram(programName, 'Network security fundamentals');
      await programs.openEditForm(programName);
    ...
    test('TC-018 — Description can be cleared if optional', async () => {
      const programName = uniqueName('Robotics 101');

      await programs.createProgram(programName, 'Introductory robotics curriculum');
      await programs.openEditForm(programName);
    ...
    test('TC-019 — Double-click on Save does not create duplicate updates or records', async () => {
      const programName = uniqueName('Quantum Computing Intro');
      const updatedName = `${programName} — Updated`;

      await programs.createProgram(programName, 'Quantum computing basics');
      await programs.openEditForm(programName);
      await programs.fillEditForm(updatedName);
    ...
    test('TC-020 — Concurrent edit by another user is handled gracefully', async ({ browser, page }) => {
      const programName = uniqueName('Web Development 2026');
      ...
      await programs.createProgram(programName, 'Original shared program');
